...
export const AddSection = (props: { createNewSection: (args: { name: string, type: "cards" | 'text', initialValue: string }) => void }) => {
  ...
  return (
    <div>
      <div className="grid grid-flow-col">
        <SectionNameSelector selectedName={state.name} sections={Object.keys(sections)} onChange={(e) => {
        ...
        <TypeSelector disabled={!!sections[state.name]} type={state.type} setType={(t) => setState({ ...state, type: t })} />
      </div>

      {!state.name ? null : state.type === 'cards'
        ? <AddCardButton onAdd={id => {
...
const SectionNameSelector = (props: { selectedName: string, sections: string[], onChange: (s: string) => void }) => {
...
const TypeSelector = (props: { type: "text" | "cards", setType: (s: "text" | "cards") => void, disabled: boolean }) => {
  return <RadioGroup
    value={props.type}
